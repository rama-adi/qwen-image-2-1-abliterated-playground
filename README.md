...
Reference uploads are saved under the output directory’s `references/` folder. The **Saved references** picker also lists reference files from earlier renders. Select a history image to view its **Experiment settings** card and use **Copy all settings** for JSON containing prompts, actual seed, CFG, dimensions, adapter details, and recorded runtime settings. History selection leaves the editor unchanged; older missing settings appear as `null`. Copies contain reference IDs, not image data.

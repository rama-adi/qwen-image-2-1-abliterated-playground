...
Left-side settings stay unchanged when generating and are saved within the browser tab across refreshes (saved references and uploaded LoRAs can be selected again from their pickers). **Seed `0` picks a random seed for each render**; the actual seed is saved with the image and shown in history. A nonzero seed is reproducible with the same settings.

Reference uploads are saved under the output directory’s `references/` folder. The **Saved references** picker also lists reference files from earlier renders. Select a history image to view its **Experiment settings** card and click **Reuse all settings** to restore its recorded prompts, character cards, actual seed, render controls, reference roles, and LoRA selection into the left sidebar. Missing reference/LoRA files must be restored first. Pod environment settings remain configured on the Pod. History selection leaves the editor unchanged; older missing settings appear as `null`. The card also displays the recorded settings as JSON.
...
RunPod loads the adapter through Diffusers/PEFT and executes its weights in BF16. Mac passes it to stable-diffusion.cpp alongside the quantized base model. Adapter compatibility is checked by the selected engine on generation; a structurally valid upload does not establish model compatibility. Only one adapter is active at a time. RunPod also supports ComfyUI Qwen 2.1 DoRA (including the Civitai fix): magnitude vectors are preserved and fused gate/up layers are split for Diffusers. The current Mac engine does not implement DoRA magnitude tensors, so those adapters are rejected on Mac instead of partially applied. LoKr is not supported. Same-adapter RunPod renders reuse the warm pipeline; changing the adapter or strength reloads it. Image metadata records the adapter name, SHA-256, and strength.

After uploading `qwen-image-2.1-fix-1.0-comfy.safetensors`, click **Use recommended settings**. This preserves your **scene and canvas size**, selects the Fix adapter at strength **1**, and applies the supplied workflow’s seed **79**, **20 steps**, **CFG 3**, **SEEDS-2 / SGM Uniform**, negative prompt, **APG** (eta 1, norm 10, momentum 0.3), and **FreSca** (low 1, high 2, cutoff 8). It also enables the workflow’s prompt expansion using the loaded Heretic encoder: 512 tokens, temperature 0.7, top-k 64, top-p 0.95, min-p 0.05, seed 0, thinking off. No extra environment variables are needed.

These controls require RunPod / Diffusers; APG and FreSca require CFG greater than 1. SEEDS-2 evaluates the denoiser twice per nonfinal step (39 evaluations for 20 steps), so expect it to take longer than Euler. Settings are recorded in history and restored by **Reuse all settings**. The workflow JSON does not contain the adapter weights. RunPod retains unquantized BF16 and the Heretic encoder instead of the workflow’s INT8 models, so matching the settings does not imply identical pixels across backends.
...
CI tests request validation, backend separation, rejection of quantized checkpoint tensors before casting, rewrite parsing, worker cleanup, authenticated WebSocket streaming, per-step event ordering, and reconnection. The workflow validates the Linux AMD64 image inside a Docker build stage, including model-class imports, numerical DoRA conversion, and authenticated HTTP startup. BuildKit then pushes directly to GHCR and reuses the `buildcache` registry tag, avoiding a full Docker load/re-push. The final image depends on the validation stage, so failed checks prevent publication. The first cache-seeding build can still be slow; application-only updates reuse dependency layers. GitHub-hosted CI has no NVIDIA GPU: its smoke test explicitly skips CUDA preflight and model downloads, and **does not certify GPU inference or image quality**. The GPU smoke script supplies that additional check on your deployed Pod.
